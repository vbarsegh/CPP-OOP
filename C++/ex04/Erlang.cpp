...
void erlang(double erlangi_xi[], double arr_r[], int *count, const double T, int hayteri_qanak);
...
void	get_that_r_ery_xary_kongruent(int n0, int lambda, int m,int myu)
{
    ...
    double	erlangi_xi[1000];
    erlang(erlangi_xi, arr_r, &count, T, hayteri_qanak);
...
{
    double ri_mul ;
    const int k = 3;//kori dzevi parametr
    ...
    while(hayteri_qanak)
    {
        ...
        erlangi_xi[i] = foo * log(ri_mul);
        cout << "erlangi i-in = " << erlangi_xi[i] << endl;
        i++;
        hayteri_qanak--;
    }
    cout << "i = " << i << endl;
